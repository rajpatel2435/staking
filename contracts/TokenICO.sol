...
function buyToken(uint256 _tokenAmount) public payable{
    ...
    require(_tokenAmount <= token.balanceOf(msg.sender), "Not enough tokens");

    require(token.transfer(msg.sender,_tokenAmount*1e18), "Failed to transfer tokens");
...
function withdrawAllTokens() public onlyOwner{
    ERC20 token = ERC20(tokenAddress);
    require(token.transfer(msg.sender, token.balanceOf(msg.sender)), "Failed to transfer tokens");
}
